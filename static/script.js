document.addEventListener('DOMContentLoaded', () => {
    ...
    window.hackerDosActive = false;
    const btnDos = document.getElementById('btn-dos-attack');
    if (btnDos) {
        btnDos.addEventListener('click', () => {
            window.hackerDosActive = !window.hackerDosActive;
            if (window.hackerDosActive) {
                btnDos.innerText = "🔌 RECONNECT";
                btnDos.style.backgroundColor = "#fff";
                btnDos.style.color = "#000";
            } else {
                btnDos.innerText = "✂ CUT WIRE";
                btnDos.style.backgroundColor = "";
                btnDos.style.color = "";
            }
        });
    }
    btnExchange.addEventListener('click', async () => {
        ...
        if (window.hackerDosActive) {
            ...
            if (hackerRow) hackerRow.innerHTML = "<span style='color:#f43f5e'>[DoS ATTACK] CONNECTION SEVERED. HANDSHAKE TIMEOUT.</span>";
            log("⚠ CRITICAL FAILURE: NETWORK UNREACHABLE (DoS DETECTED)");
            document.getElementById('alice-status-dot').className = "status-dot error";
            return;
        }
    ...
    async function updateNetworkStats() {
        ...
        try {
            ...
            if (data.bytes_sent) {
                ...
                drawNetworkGraph();
                const hackerInference = document.getElementById('hacker-net-inference');
                if (hackerInference) {
                    const totalSpeed = txSpeed + rxSpeed;
                    if (totalSpeed > 1000000) {
                        hackerInference.innerText = "⚠ ALERT: Large Data Transfer (Video/ISO/DB Dump?)";
                        hackerInference.style.color = "#f43f5e";
                    } else if (txSpeed > 5000 && rxSpeed < 500) {
                        hackerInference.innerText = "⚠ SUSPICIOUS: Data Exfiltration (Upload >> Download)";
                        hackerInference.style.color = "#ffa500";
                    } else if (totalSpeed < 100) {
                        hackerInference.innerText = "IDLE: Keep-Alive Packets";
                        hackerInference.style.color = "#555";
                    } else {
                        hackerInference.innerText = "ENCRYPTED: HTTPS/TLS Traffic - Content Unknown";
                        hackerInference.style.color = "#10b981";
                    }
                }
            }
        } catch (e) {
    ...
    const btnCrack = document.getElementById('btn-hacker-crack');
    const crackLog = document.getElementById('hacker-crack-log');
    if (btnCrack) {
        btnCrack.addEventListener('click', async () => {
            const pwd = document.getElementById('pwd-input').value;
            if (!pwd) {
                crackLog.innerHTML = "<span style='color:#f00'>Error: No password to attack.</span>";
                return;
            }
            crackLog.innerHTML = "";
            ...
            logLine("> INITIALIZING ATTACK...", "#f43f5e");
            btnCrack.disabled = true;
            const commonList = ["123456", "password", "admin", "welcome", "12345678"];
            for (let i = 0; i < 10; i++) {
                await new Promise(r => setTimeout(r, 50));
                logLine(`Testing: ${Math.random().toString(36).slice(-6)}`, "#555");
            }
            if (commonList.includes(pwd.toLowerCase())) {
                logLine(`[MATCH] ${pwd}`, "#07ffe9");
            } else {
                const bits = parseInt(document.getElementById('pwd-bits').innerText || "0");
                if (bits < 40) {
                    logLine(`[WEAK] ${pwd}`, "#ffa500");
                } else {
                    logLine(`[FAILED] Entropy too high (${bits} bits).`, "#f43f5e");
                }
            }
            btnCrack.disabled = false;
            btnCrack.innerText = "Start Dictionary Attack";
        });
    }
...
const btnMerkleTamper = document.getElementById('btn-merkle-tamper');
if (btnMerkleTamper) {
    btnMerkleTamper.addEventListener('click', () => {
        const ta = document.getElementById('merkle-leaves');
        const log = document.getElementById('hacker-merkle-log');
        if (!ta || !ta.value.trim()) return;
        const lines = ta.value.split('\n');
        if (lines.length === 0) return;
        const idx = Math.floor(Math.random() * lines.length);
        const original = lines[idx];
        const tampered = original + " [CORRUPTED]";
        lines[idx] = tampered;
        ta.value = lines.join('\n');
        log.innerText = `> TAMPERED: Line ${idx + 1} modified. Tree Root will change!`;
        ta.style.border = "1px solid #f43f5e";
        setTimeout(() => ta.style.border = "1px solid rgba(255,255,255,0.1)", 1000);
    });
}
window.runLabMerkle = async function () {
    const text = document.getElementById('merkle-leaves').value;
    const leaves = text.split('\n').filter(x => x.trim().length > 0);
    if (leaves.length === 0) return alert("Enter some data first.");
    const viz = document.getElementById('merkle-viz');
    const rootDisplay = document.getElementById('merkle-root');
    viz.innerText = "Building Tree...";
    const res = await fetch('/api/lab/merkle/build', {
        method: 'POST', body: JSON.stringify({ leaves }), headers: { 'Content-Type': 'application/json' }
    });
    const data = await res.json();
    rootDisplay.innerText = data.root;
    document.getElementById('merkle-proof-idx').max = leaves.length - 1;
    let output = "ROOT: " + data.root.substring(0, 16) + "...\n";
    output += "  |\n";
    data.levels.forEach((lvl, i) => {
        output += `  [L${i}] Size: ${lvl.length} nodes\n`;
    });
...
const btnZkpCheat = document.getElementById('btn-zkp-impersonate');
window.hackerZkpCheat = false;
if (btnZkpCheat) {
    btnZkpCheat.addEventListener('click', () => {
        window.hackerZkpCheat = true;
        document.getElementById('hacker-zkp-log').innerText = "> CHEAT MODE: Generating Fake Proofs...";
        runLabZkp();
    });
}
window.runLabZkp = async function () {
    const secretInput = document.getElementById('zkp-secret');
    let secret = parseInt(secretInput.value);
    const log = document.getElementById('zkp-log');
    const resBox = document.getElementById('zkp-result');
    if (window.hackerZkpCheat) {
        secret = 0;
        log.innerHTML = "<span style='color:#f43f5e'>[IMP] Prover Commits: Random value (No secret knowledge)</span><br>";
        await new Promise(r => setTimeout(r, 800));
        log.innerHTML += "Verifier Challenge: c = ?<br>";
        await new Promise(r => setTimeout(r, 800));
        const success = Math.random() > 0.5;
        if (success) {
            log.innerHTML += "<span style='color:#f43f5e'>[IMP] Prover Response: Guessed correctly!</span><br>";
            resBox.innerText = "VERIFIED (LUCK)";
            resBox.style.color = "#ffa500";
            document.getElementById('hacker-zkp-log').innerText += "\n> Round Passed (Lucky Guess). Try again!";
        } else {
            log.innerHTML += "<span style='color:#f43f5e'>[IMP] Prover Response: WRONG!</span><br>";
            resBox.innerText = "REJECTED";
            resBox.style.color = "#f43f5e";
            document.getElementById('hacker-zkp-log').innerText += "\n> CAUGHT! Impersonation Failed.";
        }
        window.hackerZkpCheat = false;
        return;
    }
    if (!secret) return alert("Enter a secret number (x).");
    log.innerHTML = "Prover: Committing (g^r mod p)...<br>";
    resBox.innerText = "--"; resBox.style.color = "#fff";
    const res = await fetch('/api/lab/zkp/prove', {
        method: 'POST', body: JSON.stringify({ x: secret }), headers: { 'Content-Type': 'application/json' }
    });
    const data = await res.json();
    log.innerHTML += `Verifier: Challenge c=${data.challenge}<br>`;
    log.innerHTML += `Prover: Response s=${data.response}<br>`;
    if (data.verified) {
        resBox.innerText = "VERIFIED";
...
window.hackerQkdActive = false;
const btnQkdHack = document.getElementById('btn-qkd-intercept');
if (btnQkdHack) {
    btnQkdHack.addEventListener('click', () => {
        window.hackerQkdActive = !window.hackerQkdActive;
        if (window.hackerQkdActive) {
            btnQkdHack.innerText = "🕵️ DISABLE INTERCEPT";
            btnQkdHack.style.backgroundColor = "#fff";
            btnQkdHack.style.color = "#000";
            document.getElementById('hacker-qkd-log').innerText = "> INTERCEPTING PHOTONS...";
        } else {
            btnQkdHack.innerText = "🕵️ ENABLE INTERCEPT ATTACK";
            btnQkdHack.style.backgroundColor = "";
            btnQkdHack.style.color = "";
            document.getElementById('hacker-qkd-log').innerText = "";
        }
    });
}
window.runLabQkd = async function () {
    ...
    window.bobBases.forEach((base, i) => {
        ...
        if (window.hackerQkdActive) {
            ...
            d.style.boxShadow = "0 0 5px #f43f5e";
        }
        if (window.aliceBases[i] !== base) {
            measuredBit = Math.random() > 0.5 ? 1 : 0;
            d.style.opacity = '0.5';
        } else {
            if (window.hackerQkdActive && measuredBit !== bit) {
                d.style.borderColor = '#f43f5e';
                d.innerHTML += `<div style="color:#f43f5e; font-size:1rem; position:absolute; top:-10px; right:-5px;">⚠</div>`;
            } else {
